...
小事（进 core，跨端同步），最后写一篇不超过五句的梦记。**searcher** 在 ⌥空格
接到一句完整的问题时出动（agentic RAG）：拿着 search_moments、grep_notes、
read_note、人物卡和 mdfind 这些工具多轮翻找，两句话回答，顺口说线索出自哪。
聊天在两端都一样：persona 加上最近的小传当上下文，带一个 save_moment 工具，
值得记的它自己存，闲聊不存。
...
每天至多三条，才进 CRDT 跨端。人物卡和梦记算 dream 的工作记忆，也留在本机。
...
记得，都算在问。聊天里的 save_moment 由模型自主调用，存了也不打断对话去提。
